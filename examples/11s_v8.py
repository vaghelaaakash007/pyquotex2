...
class Indicators:
    ...
    def stochastic(highs, lows, closes, k_period=14, d_period=3, smooth=3):
        ...
        %K = where the close sits inside the high/low range of the last
        k_period candles, smoothed. %D is the moving average of %K.
        """
        n = len(closes)
...
ASSET_CATEGORY_ORDER = ["Currencies", "Crypto", "Metals", "Commodities", "Indices", "Stocks"]

_METAL_TOKENS = ("XAU", "XAG", "XPT", "XPD", "GOLD", "SILVER", "PLATINUM", "PALLADIUM")
_INDEX_TOKENS = ("SP500", "SPX", "NDX", "NASDAQ", "DJI", "DOW", "DAX", "FTSE", "NIKKEI",
                 "CAC", "IBEX", "HSI", "ASX", "STOXX", "AUS200", "US30", "US100", "US500",
                 "JPN225", "UK100", "GER30", "GER40", "F40", "E35", "AEX", "SMI", "D30")
_ENERGY_TOKENS = ("CRUDE", "BRENT", "WTI", "USOIL", "UKOIL", "NGAS", "NATGAS", "OIL")


def classify_asset(name, broker_category=""):
    """Split the broker's coarse grouping into the buckets a trader actually
    thinks in: currencies / crypto / metals / energy / indices / stocks."""
    upper = str(name).upper().replace("_OTC", "")
    cat = str(broker_category or "").lower()

    if any(tok in upper for tok in _METAL_TOKENS):
        return "Metals"
    if any(tok in upper for tok in _ENERGY_TOKENS):
        return "Commodities"
    if any(tok in upper for tok in _INDEX_TOKENS):
        return "Indices"

    if cat in ("forex", "currency", "currencies"):
        return "Currencies"
    if cat in ("cryptocurrency", "crypto", "cryptocurrencies"):
        return "Crypto"
    if cat in ("commodity", "commodities", "energy"):
        return "Commodities"
    if cat in ("index", "indices"):
        return "Indices"
    if cat in ("stock", "stocks", "equity", "equities"):
        return "Stocks"
    return "Stocks"


def build_asset_catalog(instruments, payouts):
    """-> {'OTC': {category: [(asset, payout), ...]}, 'REAL': {...}}

    Only instruments the broker currently flags as open (index 14) are listed.
    """
    catalog = {"OTC": {}, "REAL": {}}
    seen = set()
    for i in instruments or []:
        if len(i) <= 14:
            continue
        name = i[1]
        if not i[14] or name in seen:
            continue
        payout = payouts.get(name, 0)
        if payout <= 0:
            continue
        seen.add(name)
        market = "OTC" if str(name).lower().endswith("_otc") else "REAL"
        category = classify_asset(name, i[3] if len(i) > 3 else "")
        catalog[market].setdefault(category, []).append((name, payout))

    for market in catalog:
        for category in catalog[market]:
            catalog[market][category].sort(key=lambda x: (-x[1], x[0]))
    return catalog


def select_assets_interactive(catalog, console):
    """Terminal menu: market -> categories -> how many. Returns a list of assets."""
    total_otc = sum(len(v) for v in catalog["OTC"].values())
    total_real = sum(len(v) for v in catalog["REAL"].values())

    if total_otc + total_real == 0:
        console.print("[bold red]No open assets reported by the broker.[/]")
        return []

    console.print()
    console.print(Panel(
        "Pick what to track. Fewer assets means faster startup and less noise.",
        title="🌐 ASSET SELECTION", title_align="left", border_style="cyan",
    ))

    # ---------------- market ----------------
    console.print("\n[bold]Market[/]")
    console.print(f"  [cyan]1[/]  🕐 OTC only          [dim]({total_otc} open — synthetic, 24/7)[/]")
    console.print(f"  [cyan]2[/]  🏛️  Real market only  [dim]({total_real} open — exchange hours)[/]")
    console.print(f"  [cyan]3[/]  🌍 Both              [dim]({total_otc + total_real} open)[/]")
    market_choice = Prompt.ask("Choose market", choices=["1", "2", "3"], default="1")
    markets = {"1": ["OTC"], "2": ["REAL"], "3": ["OTC", "REAL"]}[market_choice]

    # ---------------- categories ----------------
    available = {}
    for market in markets:
        for category, items in catalog[market].items():
            available.setdefault(category, []).extend(items)
    for category in available:
        available[category].sort(key=lambda x: (-x[1], x[0]))

    ordered = [c for c in ASSET_CATEGORY_ORDER if available.get(c)]
    if not ordered:
        console.print("[bold red]Nothing open in that market right now.[/]")
        return []

    console.print("\n[bold]Categories[/] [dim](comma-separated numbers, or 'all')[/]")
    for idx, category in enumerate(ordered, 1):
        items = available[category]
        best = items[0][1] if items else 0
        console.print(
            f"  [cyan]{idx}[/]  {ASSET_CATEGORY_EMOJI.get(category, '📁')} {category:<12} "
            f"[dim]({len(items)} open, best payout {best}%)[/]"
        )
    raw = Prompt.ask("Choose categories", default="all").strip().lower()
    if raw in ("all", "*", ""):
        chosen_cats = ordered
    else:
        chosen_cats = []
        for part in raw.replace(" ", "").split(","):
            if part.isdigit() and 1 <= int(part) <= len(ordered):
                cat = ordered[int(part) - 1]
                if cat not in chosen_cats:
                    chosen_cats.append(cat)
        if not chosen_cats:
            console.print("[yellow]Nothing recognised — using all categories.[/]")
            chosen_cats = ordered

    pool = []
    for category in chosen_cats:
        pool.extend(available[category])
    pool.sort(key=lambda x: (-x[1], x[0]))

    console.print(f"\n[bold]{len(pool)} assets[/] match that selection.")

    # ---------------- how many ----------------
    console.print("\n[bold]Selection mode[/]")
    console.print(f"  [cyan]1[/]  ✅ Track all {len(pool)}          [dim](slower startup)[/]")
    console.print("  [cyan]2[/]  🏅 Top N by payout")
    console.print("  [cyan]3[/]  ✋ Pick individually")
    mode = Prompt.ask("Choose", choices=["1", "2", "3"], default="2" if len(pool) > 20 else "1")

    if mode == "1":
        selected = [a for a, _ in pool]
    elif mode == "2":
        n = IntPrompt.ask("How many", default=min(15, len(pool)))
        selected = [a for a, _ in pool[:max(1, min(n, len(pool)))]]
    else:
        ...
        cat_of = {}
        for category in chosen_cats:
            for a, _ in available[category]:
                cat_of[a] = category
        for idx, (a, payout) in enumerate(pool, 1):
            market_tag = "🕐 OTC" if a.lower().endswith("_otc") else "🏛️ REAL"
            table.add_row(str(idx), format_asset_name(a),
                          f"{ASSET_CATEGORY_EMOJI.get(cat_of.get(a, ''), '')} {cat_of.get(a, '')}",
                          f"{payout}%", market_tag)
        console.print(table)
        raw = Prompt.ask("Enter numbers (e.g. 1,3,7-10)", default="1-10").replace(" ", "")
        picked = []
        for part in raw.split(","):
            if "-" in part:
                a_, _, b_ = part.partition("-")
                if a_.isdigit() and b_.isdigit():
                    picked.extend(range(int(a_), int(b_) + 1))
            elif part.isdigit():
                picked.append(int(part))
        selected = [pool[i - 1][0] for i in picked if 1 <= i <= len(pool)]
        if not selected:
            console.print("[yellow]Nothing valid picked — using the top 10 by payout.[/]")
            selected = [a for a, _ in pool[:10]]
    ...
    for a in selected:
        if a not in seen:
            seen.add(a)
            ordered_selection.append(a)

    console.print(f"\n[bold green]✅ Tracking {len(ordered_selection)} asset(s).[/]")
    est = len(ordered_selection) * 3
    console.print(f"[dim]Estimated history load: about {est}s (fetches are serialised — "
                  f"the broker API keeps candle data in one shared slot).[/]\n")
...
class OscillatorSuite:
    """
    Dedicated RSI + Stochastic + Bollinger analysis.

    Reading is mean-reversion at the extremes: an oscillator pinned at an
    extreme is treated as a signal that the move is stretched, not that it
    will continue. That is the conventional short-timeframe reading, but it
    is an assumption, not a law — check it against the backtest calibration
    ...
    def analyze(closes, highs, lows, regime="UNKNOWN", trend_dir=0):
        ...
        if rsi is not None:
            ...
            if rsi <= OscillatorSuite.RSI_OVERSOLD:
                ...
                out["notes"].append(f"🟢 RSI {rsi:.0f} oversold (<{OscillatorSuite.RSI_OVERSOLD})")
            elif rsi >= OscillatorSuite.RSI_OVERBOUGHT:
                out.update(rsi_state="overbought", rsi_emoji="🔴", rsi_dir=-1)
                out["notes"].append(f"🔴 RSI {rsi:.0f} overbought (>{OscillatorSuite.RSI_OVERBOUGHT})")
            elif rsi >= 55:
                out.update(rsi_state="bullish", rsi_emoji="🔼")
                out["notes"].append(f"🔼 RSI {rsi:.0f} above midline")
            elif rsi <= 45:
                out.update(rsi_state="bearish", rsi_emoji="🔽")
                out["notes"].append(f"🔽 RSI {rsi:.0f} below midline")
            else:
        ...
        if k_line and d_line and len(k_line) >= 2 and len(d_line) >= 2:
            ...
            if k <= OscillatorSuite.STOCH_OVERSOLD and cross_up:
                out.update(stoch_state="bullish cross (oversold)", stoch_emoji="🟢", stoch_dir=1)
                out["notes"].append(f"🟢 Stochastic %K {k:.0f} crossed above %D in oversold zone")
            elif k >= OscillatorSuite.STOCH_OVERBOUGHT and cross_down:
                out.update(stoch_state="bearish cross (overbought)", stoch_emoji="🔴", stoch_dir=-1)
                out["notes"].append(f"🔴 Stochastic %K {k:.0f} crossed below %D in overbought zone")
            elif k <= OscillatorSuite.STOCH_OVERSOLD:
                out.update(stoch_state="oversold", stoch_emoji="🟩", stoch_dir=1)
                out["notes"].append(f"🟩 Stochastic %K {k:.0f} oversold")
            elif k >= OscillatorSuite.STOCH_OVERBOUGHT:
                out.update(stoch_state="overbought", stoch_emoji="🟥", stoch_dir=-1)
                out["notes"].append(f"🟥 Stochastic %K {k:.0f} overbought")
            elif cross_up:
        ...
        if upper is not None and mid:
            ...
            if len(recent_widths) >= 10:
                ...
                # A real squeeze is a clear contraction, not just the lowest of
                # a noisy sample — require well below the recent median.
                if median_w > 0 and width <= median_w * 0.70 and width <= min(recent_widths):
                    out["squeeze"] = True
                    out["notes"].append(
                        f"🎯 Bollinger squeeze — width {width:.3f}% vs {median_w:.3f}% median (20 candles)"
                    )

            if percent_b <= 0.0:
                out.update(bb_state="below lower band", bb_emoji="🟢", bb_dir=1)
                out["notes"].append(f"🟢 Price pierced the lower Bollinger band (%B {percent_b*100:.0f})")
            elif percent_b >= 1.0:
                out.update(bb_state="above upper band", bb_emoji="🔴", bb_dir=-1)
                out["notes"].append(f"🔴 Price pierced the upper Bollinger band (%B {percent_b*100:.0f})")
            elif percent_b <= 0.20:
                out.update(bb_state="near lower band", bb_emoji="🟩", bb_dir=1)
                out["notes"].append(f"🟩 Price near the lower Bollinger band (%B {percent_b*100:.0f})")
            elif percent_b >= 0.80:
                out.update(bb_state="near upper band", bb_emoji="🟥", bb_dir=-1)
                out["notes"].append(f"🟥 Price near the upper Bollinger band (%B {percent_b*100:.0f})")
            else:
                out.update(bb_state="mid-band", bb_emoji="⚪")

        # ---------------- combine, regime-aware ----------------
        # An oscillator pinned at an extreme means opposite things depending on
        # the regime. In a trend it usually means "strong", not "about to
        # reverse" — reading it as a reversal is how oscillator strategies bleed
        # money in trends. So the reversal reading is only used in a range.
        ...
        if trending:
            # Extremes against the trend are ignored, not traded.
            ...
            if bb_dir == -trend_dir:
                bb_dir = 0
            # A pullback that resets and turns back with the trend is the setup.
            if rsi is not None:
                if trend_dir > 0 and 40 <= rsi <= 60:
                    rsi_dir = 1
                    out["notes"].append(f"🔼 RSI {rsi:.0f} pulled back to the midline inside an uptrend")
                elif trend_dir < 0 and 40 <= rsi <= 60:
                    rsi_dir = -1
                    out["notes"].append(f"🔽 RSI {rsi:.0f} pulled back to the midline inside a downtrend")
        ...
        if regime == "HIGH_VOLATILITY":
            # Bands and oscillators whipsaw when volatility spikes.
            out["strength"] *= 0.5
            out["notes"].append("⚠️ High volatility — oscillator weight halved")
        return out
...
class ProOTCEngine:
    ...
    def _momentum_engine(closes, highs, lows, volumes, atr_pct):
        ...
        # RSI moved to OscillatorSuite (its own scored component)
        ...
        # Stochastic moved to OscillatorSuite (its own scored component)
    ...
    def compute(tracker, asset=None, adx_threshold=None, atr_min_pct=None, atr_max_pct=None,
                min_ai_score=None, self_learning_multiplier=1.0, strictness=None, expiry_seconds=60):
        """
        ...
        # Only an unbroken run of closed candles is usable: every period-based
        # indicator assumes evenly spaced bars.
        candles, feed = tracker.series()
        empty_extra["data_quality"] = feed
        if len(candles) < ProOTCEngine.MIN_CANDLES:
            base = {
                "direction": "WAIT", "label": "Collecting data", "confidence": 0, "quality": "WAIT",
                "reasons": [f"Need {ProOTCEngine.MIN_CANDLES - len(candles)} more contiguous closed candles"
                            + (f" ({feed['gaps']} gap(s) in the feed)" if feed["gaps"] else "")],
        ...
        # ---------------- feed integrity (binary options are decided on ONE
        # candle, so a stale or tick-starved candle is a real risk) ----------
        age = feed.get("age_s")
        if age is not None and age > 90:
            vetoes.append(f"Stale feed: newest closed candle is {age}s old — no live ticks arriving")
        if feed.get("avg_ticks", 0) and feed["avg_ticks"] < 5:
            vetoes.append(
                f"Thin feed: {feed['avg_ticks']} ticks/candle on average — "
                f"OHLC is unreliable at this tick density"
            )
        if feed.get("empty_candles", 0) >= 3:
            vetoes.append(f"{feed['empty_candles']} of the last {feed['usable']} candles had almost no ticks")

        # ---------------- expiry horizon (binary-option specific) -------
        # Entry is at candle close, so a 60s option is a bet on the NEXT 1m
        # candle. If the expiry spans several candles the single-candle
        # reading no longer matches what is actually being predicted.
        horizon_candles = max(1, int(round(expiry_seconds / 60.0)))
        empty_extra["horizon_candles"] = horizon_candles
        if horizon_candles > 1:
            reasons.append(
                f"⏱ Expiry {expiry_seconds}s spans ~{horizon_candles} candles — "
                f"confirming the direction holds over that window"
            )
            recent_dirs = []
            for back in range(horizon_candles):
                idx = -1 - back
                if len(candles) > abs(idx):
                    cc = candles[idx]
                    recent_dirs.append(1 if cc["close"] > cc["open"] else (-1 if cc["close"] < cc["open"] else 0))
            if recent_dirs and len(set(d for d in recent_dirs if d != 0)) > 1:
                missing.append("multi-candle horizon agreement")
...
class TradeConfig:
    def __init__(self):
        ...
        # GLOBAL = escalate on the next trade whatever the asset (classic).
        # ASSET  = keep a separate ladder per pair, wait for that pair again.
        self.martingale_scope = "GLOBAL"
        # Settle from our own ticks at expiry so the next (escalated) entry can
        # be placed on the very next candle instead of waiting a full minute.
        self.fast_settle = True
        self.duration = 60
        # One position at a time: the bot picks the single strongest asset
        # each candle and waits for it to close before entering again.
        # Raise this only if you accept simultaneous exposure.
        self.max_concurrent_trades = 1
        # A 60% payout needs a 62.5% win rate to break even; a 94% payout needs
        # 51.5%. Trading low-payout assets is strictly worse for the same signal.
...
class TradeManager:
    ...
    @staticmethod
    def _norm_dir(value):
        """Signals say BUY/SELL, orders say call/put. They were compared raw,
        so the martingale lock never matched and the escalation was wiped."""
        v = str(value).strip().lower()
        if v in ("buy", "call", "up"):
            return "call"
        if v in ("sell", "put", "down"):
            return "put"
        return v
    ...
    def get_next_stake(self, asset, balance=None, signal_confidence=None):
        ...
        scope = getattr(self.config, "martingale_scope", "GLOBAL")
        raw_step = self.global_step if scope == "GLOBAL" else self.steps[asset]
        step = min(raw_step, self.config.max_stake_steps)
    ...
    def current_step(self, asset=None):
        if getattr(self.config, "martingale_scope", "GLOBAL") == "GLOBAL":
            return self.global_step
        return self.steps[asset] if asset else 0
    ...
    def record_result(self, asset, direction, amount, confidence, indicators, won, profit, opened_at, votes=None):
        ...
        prov = self._provisional.pop(asset, None)
        if prov is None:
            self.open_assets.discard(asset)
            self._apply_step(asset, direction, won)
        elif prov["won"] != won:
            # The provisional read (our own tick data at expiry) disagreed with
            # the broker. Rewind the ladder and reapply the official outcome.
            self.steps[asset] = prov["step_before"]
            self.global_step = prov["global_before"]
            self.pending_lock = prov["lock_before"]
            self._apply_step(asset, direction, won)
            self.provisional_mismatches += 1
    ...
    def settle_provisional(self, asset, direction, won):
        """Free the slot and move the stake ladder the moment the option
        expires, using our own tick stream.

        The broker's confirmation arrives a second or so after expiry — which
        is after the candle boundary where the next entry has to be placed.
        Waiting for it cost a full candle every time, so the escalated trade
        never fired straight after the loss. P&L still comes from the official
        result; this only controls the slot and the ladder.
        """
        self.open_assets.discard(asset)
        self._provisional[asset] = {
            "won": won,
            "step_before": self.steps[asset],
            "global_before": self.global_step,
            "lock_before": self.pending_lock,
        }
        self._apply_step(asset, direction, won)

    def _apply_step(self, asset, direction, won):
        mode = self.config.money_mgmt_mode
        scope = getattr(self.config, "martingale_scope", "GLOBAL")

        def _escalate():
            # GLOBAL is the classic behaviour: the next qualifying trade carries
            # the escalated stake, whatever asset it lands on. ASSET keeps a
            # separate ladder per pair and waits for that same pair to set up.
            if scope == "GLOBAL":
                if self.global_step < self.config.max_stake_steps:
                    self.global_step += 1
                    self.pending_lock = None
                    return True
                self.global_step = 0
                self.pending_lock = None
                return False
            if self.steps[asset] < self.config.max_stake_steps:
                self.steps[asset] += 1
                self.pending_lock = {"asset": asset, "direction": self._norm_dir(direction)}
                return True
            self.steps[asset] = 0
            self.pending_lock = None
            return False
...
def setup_trade_config(console, use_defaults=False):
    ...
    if config.money_mgmt_mode in ("MARTINGALE", "ANTI_MARTINGALE", "ADAPTIVE"):
        ...
        scope = Prompt.ask(
            "Escalate on the next trade regardless of asset (global), or per asset?",
            choices=["global", "asset"], default="global",
        )
        config.martingale_scope = scope.upper()

        # Show what the ladder actually costs before it is armed.
        ladder, total = [], 0.0
        for step in range(config.max_stake_steps + 1):
            stake = config.base_amount * (config.stake_multiplier ** step)
            total += stake
            ladder.append(f"step {step}: ${stake:.2f}")
        console.print(
            "[bold yellow]⚠ Stake ladder:[/] " + "  →  ".join(ladder) +
            f"\n[bold yellow]  Losing the full ladder costs ${total:.2f}[/] "
            f"({total / max(config.base_amount, 0.01):.0f}× your base stake). "
            f"A {config.max_stake_steps + 1}-loss run is not rare: at a 50% win rate it happens "
            f"roughly once every {2 ** (config.max_stake_steps + 1)} trades."
        )
        if not Confirm.ask("Arm this ladder?", default=True):
            config.money_mgmt_mode = "FLAT"
            console.print("[green]Switched to flat staking.[/]")
    ...
    config.max_concurrent_trades = IntPrompt.ask(
        "Max positions open at the same time (1 = one trade at a time)", default=1
    )
    config.max_concurrent_trades = max(1, min(10, config.max_concurrent_trades))
    config.min_payout_pct = FloatPrompt.ask(
        "Minimum payout % to allow a trade (85% => 54.1% break-even)", default=85.0
    )
...
class CandleTracker:
    ...
    def pre_populate(self, history_candles):
        ...
        for candle in history_candles:
            ...
            bucket = (ts // 60) * 60
            existing = self.candles.get(bucket)
            # Never let a history fetch overwrite a candle we built from the
            # live tick stream — the live one has the real tick count and the
            # true close we acted on.
            if existing is not None and existing.get("ticks", 0) > 0:
                continue
            self.candles[bucket] = {
                "timestamp": bucket,
        ...
        self.candle_keys = deque(sorted(set(self.candle_keys)), maxlen=self.candle_keys.maxlen)
    ...
    # Candle integrity
    # ------------------------------------------------------------------
    def close_stale_candles(self, now_ts=None):
        """Close candles by the clock, not by the arrival of the next tick.

        Previously a candle was only marked closed when the first tick of the
        NEXT minute arrived. If the stream stalled, the finished candle stayed
        'open' and was silently dropped from every indicator — so the engine
        was deciding on data one or two minutes stale.
        """
        now_ts = time.time() if now_ts is None else now_ts
        closed_any = False
        for bucket in list(self.candle_keys):
            c = self.candles.get(bucket)
            if c is not None and not c["is_closed"] and bucket + 60 <= now_ts:
                c["is_closed"] = True
                closed_any = True
        return closed_any

    def series(self, max_len=200, now_ts=None):
        """Longest CONTIGUOUS run of closed candles ending at the newest one.

        Indicators assume evenly spaced bars. If minutes are missing from the
        feed, feeding the survivors in as if they were adjacent silently
        corrupts every period-based calculation (RSI, ATR, Bollinger, ADX).
        This returns only an unbroken run, plus a quality report.
        """
        self.close_stale_candles(now_ts)
        buckets = sorted(b for b in self.candles if self.candles[b]["is_closed"])
        if not buckets:
            return [], {"gaps": 0, "usable": 0, "total": 0, "avg_ticks": 0.0,
                        "empty_candles": 0, "last_bucket": None, "age_s": None}

        run_start = len(buckets) - 1
        gaps = 0
        for i in range(len(buckets) - 1, 0, -1):
            if buckets[i] - buckets[i - 1] == 60:
                run_start = i - 1
            else:
                gaps = sum(1 for j in range(1, len(buckets)) if buckets[j] - buckets[j - 1] != 60)
                break
        contiguous = buckets[run_start:][-max_len:]
        candles = [self.candles[b] for b in contiguous]

        live_ticks = [c["ticks"] for c in candles if c.get("source") == "live"]
        avg_ticks = sum(live_ticks) / len(live_ticks) if live_ticks else 0.0
        empty = sum(1 for c in candles if c.get("source") == "live" and c["ticks"] < 3)
        last_bucket = contiguous[-1] if contiguous else None
        now_ts = time.time() if now_ts is None else now_ts
        return candles, {
            "gaps": gaps,
            "usable": len(candles),
            ...
            "age_s": int(now_ts - (last_bucket + 60)) if last_bucket else None,
        }
...
class DashboardApp:
    ...
    def _compute_one_signal(self, asset):
        ...
        if tm_cfg.engine == "PRO_OTC":
            ...
            return asset, ProOTCEngine.compute(
                ...
                expiry_seconds=tm_cfg.duration,
            )
    ...
    async def update_signals(self):
        ...
        # Recompute only for assets whose newest closed candle changed.
        # Every indicator is a function of closed candles, so recomputing all
        # of them 4x a second was pure waste — and it is what made tracking
        # the full asset list too slow before.
        stale = []
        for asset in self.active_assets:
            tracker = self.trackers.get(asset)
            if tracker is None:
                continue
            tracker.close_stale_candles()
            closed = [b for b in tracker.candle_keys if tracker.candles.get(b, {}).get("is_closed")]
            marker = closed[-1] if closed else None
            if self._signal_cache.get(asset) != marker or asset not in self.signals:
                self._signal_cache[asset] = marker
                stale.append(asset)

        # This one field is a clock reading, not an indicator — refresh it on
        # every pass so the countdown keeps ticking on cached signals.
        for asset, sig in self.signals.items():
            tracker = self.trackers.get(asset)
            if tracker is not None and isinstance(sig, dict):
                sig["seconds_to_entry"] = ProOTCEngine._seconds_to_entry(tracker)

        if not stale:
            return

        results = await asyncio.gather(
            *(asyncio.to_thread(self._compute_one_signal, asset) for asset in stale)
        )
    ...
    async def fire_entries(self, frozen_signals=None, timing_offset_ms=None):
        ...
        if tm.pending_lock is not None:
            ...
            still_valid = (
                ...
                and TradeManager._norm_dir(locked_sig["direction"]) == TradeManager._norm_dir(locked_direction)
            ...
            if still_valid:
                ...
                self.add_log(
                    ...
                    f"(step {tm.steps[locked_asset]} → 0).[/]"
                )
        ...
        for asset in self.active_assets:
            ...
            if payout and payout < min_payout:
                self.add_log(
                    f"[dim]  {format_asset_name(asset)} skipped: payout {payout}% < {min_payout:.0f}% "
                    f"(needs {100 / (1 + payout / 100):.1f}% win rate just to break even)[/]"
                )
                continue
        ...
        if len(candidates) > len(picked):
            ...
            self.add_log(
                ...
                f"taking the strongest: {format_asset_name(best[0])} ({best[1]['confidence']}%)[/]"
            )
        ...
        for asset, sig in picked:
            direction = "call" if sig["direction"] == "BUY" else "put"
            amount = tm.get_next_stake(asset, balance=balance, signal_confidence=sig["confidence"])
            step_now = tm.current_step(asset)
            if step_now > 0:
                self.add_log(
                    f"[bold magenta]  ⏫ Martingale step {step_now}/{tm.config.max_stake_steps}[/] — "
                    f"stake ${amount:.2f} (base ${tm.config.base_amount:.2f} × "
                    f"{tm.config.stake_multiplier}^{step_now})"
                )
    ...
    async def execute_trade(self, asset, direction, amount, confidence, indicators=None, votes=None):
        ...
        try:
            ...
            # The library returns (status, info), but some error paths return a
            # bare bool. Unpacking blindly raised "cannot unpack non-iterable
            # bool object", which was swallowed by the except below — so the
            # trade never reached the journal and STATS stayed empty.
            ...
            if order_id is None:
                self.add_log(
                    f"[bold red]Order placed but no order id returned[/] {format_asset_name(asset)} — "
                    f"cannot track the result."
                )
                tm.mark_closed(asset)
                return

            # Free the slot and step the ladder the moment the option expires,
            # rather than when the broker's confirmation lands ~1s later. The
            # next candle boundary IS the expiry instant, so waiting for the
            # confirmation meant every escalated entry missed its candle.
            expiry_ts = opened_at + tm.config.duration
            if isinstance(buy_info, dict) and buy_info.get("closeTimestamp"):
                try:
                    expiry_ts = float(buy_info["closeTimestamp"])
                except Exception:
                    pass
            settle_task = None
            if getattr(tm.config, "fast_settle", True):
                settle_task = asyncio.create_task(
                    self._provisional_settle(asset, direction, buy_info, expiry_ts)
                )
            ...
            if won is None:
                self.add_log(
                    f"[bold yellow]UNRESOLVED[/] {format_asset_name(asset)} (order {order_id}) — "
                    f"{source}. Not recorded in P&L; check the broker history manually."
                )
                _log_to_file(f"Unresolved result: asset={asset} order_id={order_id} reason={source}")
            ...
            self.add_log(
                ...
                f"→ {profit:+.2f} | session P&L: {tm.session_pnl:+.2f} [dim](via {source})[/]"
            )
            if tm.halted:
            ...
            _log_to_file(f"Trade execution error {asset}: {traceback.format_exc()}")
            tm.mark_closed(asset)

    async def _provisional_settle(self, asset, direction, buy_info, expiry_ts):
        """At expiry, read our own tick stream to decide the outcome, release
        the slot and move the stake ladder. Fires a fraction of a second early
        so the slot is already free when the candle-boundary tick runs.

        This is an estimate. P&L always comes from the broker's official
        result, and if the two disagree the ladder is rewound in record_result.
        """
        tm = self.trade_manager
        lead = 0.6
        delay = expiry_ts - lead - time.time()
        if delay > 0:
        ...
        if isinstance(buy_info, dict):
            try:
                open_price = float(buy_info.get("openPrice"))
            except (TypeError, ValueError):
                open_price = None
        current = self.last_price.get(asset)

        if not open_price or current is None or current == open_price:
            self.add_log(
                f"[dim]  Fast-settle skipped for {format_asset_name(asset)} "
                f"(no price move recorded) — waiting for the broker result.[/]"
            )
            return

        won = (current > open_price) if direction == "call" else (current < open_price)
        tm.settle_provisional(asset, direction, won)
        step = tm.current_step(asset)
        self.add_log(
            f"[dim]  ⚡ Fast-settle {format_asset_name(asset)}: "
            f"{'WIN' if won else 'LOSS'} (est.) — slot freed, ladder at step {step}. "
    ...
    async def _resolve_trade_result(self, asset, direction, amount, order_id, buy_info, balance_before):
        """
        Resolve a trade outcome through four independent channels, in order of
        reliability. Any one of them succeeding is enough to record the trade.

        The old code relied only on client.check_win(), which (a) returns a
        bare bool, not (status, profit), and (b) blocks forever if the
        broker's "deals" websocket frame never arrives for that id.
        """
        tm = self.trade_manager
        duration = tm.config.duration
        api = getattr(self.client, "api", None)

        # --- 1. websocket deal frame, polled directly (no blocking, no stdout)
        ...
        while time.time() < deadline:
            ...
            if info and info.get("game_state") == 1:
                ...
                profit = None
                try:
                    # Prefer the per-order figure from history; the library's
                    # shared profit_in_operation is overwritten by whichever
                    # deal closed last, which is wrong with concurrent trades.
        ...
        # --- 2. server-side trade history, matched by ticket
        for _ in range(6):
            try:
        ...
        # --- 3. open price vs. current price (the library's own fallback)
        try:
            ...
            try:
                ...
                if prices:
                    ...
                    if current != open_price:
                        ...
                        payout = self.payouts.get(asset, 0) or tm.config.kelly_payout_pct
                        profit = amount * (payout / 100.0) if won else -amount
                        return won, profit, "price comparison (estimated payout)"
            except Exception:
                pass
        ...
        try:
            await self._refresh_balance()
            balance_after = self._balance_float()
            if balance_before is not None and balance_after is not None:
                delta = balance_after - balance_before
                if abs(delta) > 0.009:
                    return delta > 0, delta, "balance delta"
        except Exception:
            pass

        return None, None, "no result from websocket, history, price or balance"
    ...
    def build_multi_asset_table(self):
        ...
        for category in cat_order:
            ...
            if self.max_rows:
                assets_in_cat = assets_in_cat[:self.max_rows]
            ...
            for asset in assets_in_cat:
                ...
                if rsi_val is None:
                    ...
                    rsi_style = ("bold green" if osc.get("rsi_dir") == 1
                                 else "bold red" if osc.get("rsi_dir") == -1 else "white")
                ...
                if k is None:
                    ...
                    st_style = ("bold green" if osc.get("stoch_dir") == 1
                                else "bold red" if osc.get("stoch_dir") == -1 else "white")
                ...
                if bb_pct is None:
                    ...
                    bb_style = ("bold green" if osc.get("bb_dir") == 1
                                else "bold red" if osc.get("bb_dir") == -1 else "white")
                    sq = "🎯" if osc.get("squeeze") else ""
                    bb_cell = f"{osc.get('bb_emoji', '⚪')} [{bb_style}]{max(bb_pct, 0):.0f}%[/]{sq}"

                dq = (sig or {}).get("data_quality") or {}
                if not dq:
                    feed_cell = "[dim]—[/]"
                else:
                    age, gaps = dq.get("age_s"), dq.get("gaps", 0)
                    avg_t = dq.get("avg_ticks", 0)
                    if age is not None and age > 90:
                        feed_cell = f"[bold red]🔌 {age}s[/]"
                    elif avg_t and avg_t < 5:
                        feed_cell = f"[yellow]🪫 {avg_t:.0f}t[/]"
                    elif gaps:
                        feed_cell = f"[yellow]🕳 {gaps}[/]"
                    else:
                        feed_cell = f"[green]📡 {dq.get('usable', 0)}[/]"
                ...
                table.add_row(
                    ...
                    rsi_cell, stoch_cell, bb_cell, bar_str, feed_cell, signal_str
                )
    ...
    def build_trades_table(self):
        """Full-width, emoji-formatted history of every trade this session."""
        tm = self.trade_manager
        ...
        for offset, r in enumerate(visible, start=1):
            ...
            pnl_cell = (f"[bold green]+{r['profit']:.2f}[/]" if won
                        else f"[bold red]{r['profit']:.2f}[/]")
            run_cell = (f"[bold green]{running:+.2f}[/]" if running >= 0
                        else f"[bold red]{running:+.2f}[/]")
    ...
    def build_stats_panel(self):
        ...
        if stats["trades"] == 0 and not self.unresolved_trades:
            ...
            return Panel(text, title="📊 STATS", title_align="left", box=box.SIMPLE,
        ...
        if stats["trades"] >= 5:
            ...
            gap = wr - breakeven
            text.append(
                f"({gap:+.1f}pp {'above 🟢' if gap >= 0 else 'below 🔴'})\n",
        ...
        if tm.config.money_mgmt_mode != "FLAT":
            step = tm.current_step()
            scope = getattr(tm.config, "martingale_scope", "GLOBAL").lower()
            next_stake = tm.config.base_amount * (tm.config.stake_multiplier ** step)
            icon = "⏫" if step > 0 else "▪️"
            text.append(f"{icon} {tm.config.money_mgmt_mode.title()} ({scope}): ", style="dim")
            text.append(f"step {step}/{tm.config.max_stake_steps}", style="bold magenta" if step else "bold")
            text.append(f" → next ${next_stake:.2f}\n", style="bold")
        ...
        if tm.provisional_mismatches:
            text.append(
                f"🔄 {tm.provisional_mismatches} fast-settle estimate(s) corrected by the broker\n",
                style="yellow",
            )

        if self.unresolved_trades:
            text.append(
                f"⚠️ {len(self.unresolved_trades)} unresolved — excluded from the numbers above\n",
                style="bold yellow",
            )
    ...
    async def run(self):
        ...
        try:
            with Live(layout, refresh_per_second=4, screen=True) as live:
                ...
                if self.asset == "ALL":
                    self.active_assets = sorted(self.client.codes_asset.keys())
                    self.add_log(f"Fetched full asset list ({len(self.active_assets)} instruments).")
                    layout["logs"].update(self.build_logs_panel())
                else:
                ...
                if self.asset == "ALL":
                    instruments = (self.client.api.instruments if self.client.api else None) or []
                    catalog = build_asset_catalog(instruments, self.payouts)
                    for market in catalog:
                        for category, items in catalog[market].items():
                            for name, _ in items:
                    ...
                    if self.max_assets:
                        # Non-interactive path (--max-assets given).
                        pool = []
                        for market in ("OTC", "REAL"):
                            for items in catalog[market].values():
                                pool.extend(items)
                        pool.sort(key=lambda x: (-x[1], x[0]))
                        self.active_assets = [a for a, _ in pool[:self.max_assets]]
                    else:
                        # Leave the live display so the menu can be shown, then
                        # resume it once the selection is made.
                        live.stop()
                        try:
                            self.active_assets = select_assets_interactive(catalog, console)
                        finally:
                            live.start()

                    if not self.active_assets:
                        raise RuntimeError("No assets selected — nothing to track.")

                    self.otc_count = sum(1 for a in self.active_assets if a.lower().endswith("_otc"))
                    self.add_log(
                        f"Tracking {len(self.active_assets)} asset(s): "
                        f"{self.otc_count} OTC / {len(self.active_assets) - self.otc_count} real market"
                    )
                    layout["logs"].update(self.build_logs_panel())
                ...
                # The broker API parks candle results in ONE shared slot
                # (api.candles.candles_data), so concurrent fetches overwrite
                # each other's data. They must be serialised.
                history_semaphore = asyncio.Semaphore(1)
                ...
                async def _fetch_history(asset):
                    async with history_semaphore:
                        history = None
                        try:
                            # get_candles() loops on `while self.check_connect ...`,
                            # but check_connect is a METHOD object — always truthy.
                            # With no data arriving it spins forever, which is
                            # exactly how startup used to hang. Hard timeout it.
                            if has_deep_fetch:
                                history = await asyncio.wait_for(
                                    self.client.get_historical_candles(
                                        asset, HISTORY_LOOKBACK_SECONDS, 60,
                                        timeout=FETCH_TIMEOUT, max_workers=1,
                                    ),
                                    timeout=FETCH_TIMEOUT + 5,
                                )
                            if not history:
                                history = await asyncio.wait_for(
                                    self.client.get_candles(asset, time.time(), 3 * 3600, 60),
                                    timeout=FETCH_TIMEOUT,
                                )
                        except asyncio.TimeoutError:
                            self.add_log(
                                f"[yellow]History timeout[/] {format_asset_name(asset)} "
                                f"after {FETCH_TIMEOUT}s — will build candles live instead."
                            )
                            _log_to_file(f"History fetch timeout for {asset}")
                        ...
                        done, total = history_progress["done"], len(self.active_assets)
                        filled = int(done / total * 20)
                        self.add_log(
                            f"[dim]   [{'█' * filled}{'░' * (20 - filled)}] {done}/{total} "
                            f"— {format_asset_name(asset)} ({len(history) if history else 0} candles)[/]"
                        )
                        layout["logs"].update(self.build_logs_panel())
                ...
                for idx, asset in enumerate(self.active_assets, 1):
                    ...
                    # The dashboard reads api.realtime_sentiment / realtime_price
                    # but nothing ever subscribed to them, which is why the
                    # Buy% column showed one frozen value per asset.
...
def main():
    ...
    parser.add_argument("--rows", type=int, default=15,
                        help="Max rows shown per category in the table (display only)")
    parser.add_argument("--max-assets", type=int, default=0,
                        help="Cap how many assets are tracked (0 = every open asset on the platform)")
