...
if os.name == 'posix':
    ...
    # Fix for Termux display - prevent box characters
...
# Suppress PIL warnings and fix encoding
import warnings
...
try:
    ...
    print("[INFO] Install with: pip install PySocks")
import urllib3
...
# This happens when bot is running and user tries to send manual result
async_loop_holder = {}
...
def safe_async_run(coro):
    ...
    try:
        ...
        if loop.is_running():
            # We're inside a running loop - schedule as task
...
# Unicode to ASCII converter for Termux/Kali Linux compatibility
# Fixes display issues where fancy Unicode characters show as boxes/gibberish
...
def _normalize_unicode(text):
    """Convert Unicode fancy characters to simple ASCII for terminal compatibility"""
...
def fancy_font(text):
    """Normal font with Unicode normalization"""
    return _normalize_unicode(text)  
...
async def process_signal_background(client, pair, direction, data):
    ...
    try:
        # Run process_signal in a separate thread so it doesn't block the event loop
        import concurrent.futures
        ...
        # Run in thread pool with long timeout (signal lifecycle can take 5+ minutes)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(_run_signal)
            # Don't wait - let it run in background
            # We just need to make sure it started
...
async def try_fetch(client, asset, period):
    """Fetch candles with timeout - prevents hanging on Termux/Kali"""
    try:
...
async def send_signal(client):
    ...
    # Safety: if last signal was less than 60s ago, skip (prevents spam)
    ...
    # FIXED: Removed ThreadPoolExecutor + asyncio.run() which crashes in nested event loops
    # Using asyncio.gather() for proper concurrent async execution
    ...
    # Analyze pairs concurrently using asyncio.gather (proper way for async)
    tasks = []
    pair_map = []
    ...
    while bot_running:
        try:
            ...
            if now - last_signal_time >= next_signal_interval:
                ...
                # Create new task that runs independently with exception logging
...
async def send_signal_fire_and_forget(client):
    ...
    try:
        ...
        # FIXED: Process pairs ONE BY ONE with timeout (asyncio.gather hangs on blocking I/O)
        pairs = await get_profitable_pairs(client, min_profit=70)
        if not pairs:
            print(rainbow_text("No profitable pairs found. Skipping this cycle."))
            await asyncio.sleep(10)
        ...
        for pair in pairs[:40]:
            ...
            try:
                # Analyze with 20-second timeout per pair
...
☲☲☲_ 【 𝐀𝙸 𝐒𝙸𝙶𝙽𝙰𝐋 𝐐𝚇 】☲☲☲☲"""
        ...
        # ম্যানুয়াল ফলাফল পাঠানোর লজিক - FIXED for Termux/Kali
