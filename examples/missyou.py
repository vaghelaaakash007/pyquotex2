...
def get_next_amount(base_amount, is_win, win_streak):
    """
    New money management:
    - After 1st win: amount = base * 1.5, win_streak = 1
    - After 2nd consecutive win: reset to base, win_streak = 0
    - On loss: reset to base, win_streak = 0
    """
    if is_win:
        win_streak += 1
        if win_streak >= 2:
            ...
            # First win → multiply by 1.5
            return base_amount * 1.5, win_streak
    else:
        # Loss → reset
        return base_amount, 0
...
async def single_asset_mode(config):
    ...
    while True:
        ...
        if result == "Win":
            ...
            # Update amount based on new logic
            amount, win_streak = get_next_amount(base_amount, True, win_streak)
            if win_streak == 0:
        ...
        amount, win_streak = get_next_amount(base_amount, False, win_streak)
