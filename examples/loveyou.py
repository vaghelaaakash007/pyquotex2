...
"""
    ...
    def update_last_prediction_result(self, result: str):
        """Update the last prediction entry with trade result"""
        if self.prediction_history:
            self.prediction_history[-1]['result'] = result
    
    def show_prediction_history(self):
        """Display recent prediction history with trade results"""
        ...
        table.add_column("Result", style="bold")  # New column
...
class QuantumTradingBotEnhanced:
    ...
    def __init__(self):
        ...
        # Compound strategy state (for mode 3)
        self.compound_loss_streak = 0
        self.compound_last_amount = self.trade_amount  # will be updated after amount calculation
    ...
    def get_next_amount(self, last_win: bool) -> float:
        ...
        if self.trading_mode == "1":
            ...
            # Compound strategy with loss progression:
            # - After a win: reset to base amount
            # - Loss streak: 1st loss: base*2; 2nd: previous*2.4; 3rd: previous*2.5; 4th: previous*2.6; 5th loss: reset to base.
            if last_win:
                # Reset on win
                self.compound_loss_streak = 0
            ...
            while self.is_running:
                ...
                if opportunities:
                    ...
                    if result:
                        ...
                        # Update visualizer history with result
                        self.visualizer.update_last_prediction_result(result['result'])
                        # After trade, redisplay history to show updated result
                        self.visualizer.show_prediction_history()
