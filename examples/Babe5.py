...
class ScalpingStrategyBase:
    ...
    def record_trade(self, result):
        """Called after each trade with result: 'Win', 'Loss', or 'Doji'"""
        pass


class FatherReX(ScalpingStrategyBase):
    """Triggers a CALL after 1 consecutive loss (changed from 3)"""
    def __init__(self):
        super().__init__()
        self.name = "FatherReX"
        self.description = "CALL after 1 loss (resets on win)"
        self.win_rate_target = "N/A"
        self.loss_streak = 0
    ...
    def record_trade(self, result):
        if result == "Loss":
            ...
            self.loss_streak = 0
        # Doji does not affect streak

    def get_signal(self):
        if self.loss_streak >= 1:   # 🔁 changed from >=3 to >=1
...
class StrategyManager:
    def __init__(self):
        self.strategies = {
            ...
            "11": Tick_Scalping(),
            "12": FatherReX(),   # <-- New strategy with 1‑loss trigger
        }
...
# ==================== MONEY MANAGEMENT ====================
def get_next_amount(current_amount, base_amount, is_win, win_streak, multiplier=2):
    if is_win:
        win_streak += 1
        ...
        if win_streak >= 2:
            ...
            return current_amount * multiplier, win_streak
    else:
        return base_amount, 0
...
async def single_asset_mode(config):
    ...
    while True:
        ...
        trade_count += 1
        print(f"\n📈 TRADE #{trade_count} | ${amount} | {direction.upper()} | Streak: {win_streak}")
        ...
        # 🔔 Record trade result for FatherReX and other future strategies
        if strategy_manager and strategy_manager.active_strategy:
            strategy_manager.active_strategy.record_trade(result)
...
async def shuffle_mode(config):
    ...
    amount = base_amount
    duration = 60  # default, but you can vary
    balance = await client.get_balance()
    initial_balance = balance
    trade_count = 0
    total_wins = 0
    ...
    while True:
        ...
        if strat:
            # Clear previous history for new asset
        ...
        # Execute trade
        trade_count += 1
        print(f"\n📈 TRADE #{trade_count} | ${amount} | {direction.upper()} | {asset} | Streak: {win_streak}")
        ...
        if result == "Win":
            ...
            total_profit += profit
            print(f"   ✅ WIN! +${profit:.2f} | Bal: ${balance:.2f} | {total_wins}W/{total_losses}L")
            # Shuffle uses 1.5× multiplier, reset after 2 wins
            amount, win_streak = get_next_amount(amount, base_amount, True, win_streak, multiplier=1.5)
            if win_streak == 0:
                print(f"   🔄 2 WINS RESET → ${amount}")
            else:
                print(f"   📈 ×1.5 → ${amount}")
        ...
        # Small delay before next shuffle
        await asyncio.sleep(1)
    ...
    try:
        while True:
            ...
            if signals:
                ...
                # Execute top signals
                for signal in signals[:3]:  # Max 3 per scan
