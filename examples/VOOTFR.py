...
- TRADES EXPIRE IN 5 SECONDS ONLY
- NO COUNTDOWN - EXECUTES IMMEDIATELY
"""
...
class QuantumTradingBotEnhanced:
    def __init__(self):
        ...
        # ✅ Only 5‑second timeframe – trades expire in 5s
        self.markets = {
            "EURUSD_otc": [5],
            "GBPUSD_otc": [5],
            "USDJPY_otc": [5],
    ...
    async def setup(self):
        ...
        console.print(Panel.fit(
            ...
            "[yellow]⏱️ ALL TRADES EXPIRE IN 5 SECONDS[/yellow]\n"
            "[dim]✅ No countdown – immediate execution[/dim]",
            border_style="cyan"
        ))
    ...
    async def scan_all_markets(self) -> List[Dict]:
        opportunities = []
        # Only 5‑second timeframe is used
        for asset, timeframes in self.markets.items():
            for period in timeframes:  # period is always 5
    ...
    async def execute_trade(self, asset: str, direction: str, period: int, amount: float) -> Optional[Dict]:
        try:
            ...
            await asyncio.sleep(period + 3)  # wait for expiry + buffer
    ...
    async def run(self):
        ...
        console.print("[dim]Scanning 5‑second timeframe only...[/dim]")
        ...
        try:
            while self.is_running:
                ...
                console.print(f"\n[bold red]🚨 EXECUTING IMMEDIATELY! 🚨[/bold red]")
