...
from enum import IntEnum   # <-- Added for AccountType
...
# ==================== USER CONFIG ====================
# --- NEW: Always include these two OTC pairs ---
...
def get_user_config():
    ...
    if assets_input.upper() == "ALL":
        asset_list = ["ALL"]   # placeholder – will be resolved after connection
    else:
    ...
    print(f"Account: {'REAL' if account_type == AccountType.REAL else 'DEMO'}")
    ...
    return max_trades, base_amount, stop_loss, stop_profit, strategy_manager, asset_list, account_type
...
async def trade_and_monitor():
    global client  # We will assign the client inside this function

    max_trades, base_amount, stop_loss, stop_profit, strategy_manager, asset_list, account_type = get_user_config()
    
    # Instantiate the client with the selected account type
    email, password = credentials()
    client = Quotex(
        email=email,
        password=password,
        lang="pt",
        account_type=account_type   # Pass the enum value (0 or 1)
    )
    ...
    try:
        ...
        if check_connect:
            ...
            # ---- ENSURE THE ALWAYS INCLUDED PAIRS ARE IN THE LIST (even with ALL) ----
