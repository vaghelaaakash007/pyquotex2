...
class CompoundingStrategy:
    """
    Strategy with EMA 39, Stochastic RSI (14,3,3), Support/Resistance,
    candlestick patterns, and a 3‑step compounding flow.
    """
    ...
    def update_after_trade(self, win, profit=None):
        ...
        if win:
            ...
            if profit is not None:
                self.current_stake += profit  # compound the profit
            else:
                # fallback: assume 80% payout if profit not provided
    ...
    def calculate_stochastic_rsi(prices, period=14, smooth_k=3, smooth_d=3):
        ...
        # Compute RSI for each candle in a sliding window
        rsi_series = []
    ...
    def detect_support_resistance(prices, lookback=20, threshold=0.001):
        ...
        return filter_levels(supports), filter_levels(resistances)
    ...
    def get_signal(self, candles):
        ...
        # Indicators
        ema39 = self.calculate_ema(closes, 39)
        ...
        # ----- Scoring -----
        call_score = 0
        if current_price > ema39:
            call_score += 25
        if stoch_k < 20 and stoch_k > stoch_d and prev_k <= prev_d:  # oversold + cross up
            call_score += 30
        if near_support:
            call_score += 25
        if pattern == 'bullish':
        ...
        if current_price < ema39:
            put_score += 25
        if stoch_k > 80 and stoch_k < stoch_d and prev_k >= prev_d:  # overbought + cross down
            put_score += 30
        if near_resistance:
            put_score += 25
        if pattern == 'bearish':
...
# ==================== SINGLE ASSET MODE (with Strategy) ====================
async def single_asset_mode(config):
    ...
    asset = "AUDCAD"  # default, you can change or ask user
    ...
    while True:
        ...
        if result == "Win":
            ...
            # Doji is neutral, leave stake unchanged
        else:
            total_losses += 1
...
async def trade_and_monitor():
    ...
    try:
        if trading_mode == "shuffle":
            initial, final, trades, wins, losses, profit, losses, profit, loss_, loss_amt = await shuffle_mode(config)
       amt = await shuffle_mode(config)
        else:
            initial else:
            initial, final, trades, wins, losses, final, trades, profit, loss_amt = await single_asset_mode, wins, losses, profit, loss_amt = await(config)
        
        single_asset_mode(config)
        
        final_pnl = final final_pnl = final - initial - initial
        
        print(f"\n{'='*
        
        print(f"\n{'='*60}")
        print(f"🏁 SESSION SUMMARY60}")
        print(f"🏁 SESSION SUMMARY")
        print(f"{")
        print(f"{'='*60'='*60}")
        mode_names = {"single": "Single Asset",}")
        mode_names = {"single": "Single Asset", "shuffle": "shuffle": "Shuffle Currencies"}
        print "Shuffle Cur(f"Mode: {mode_names[trencies"}
        print(f"Mode: {mode_names[trading_mode]}")
        print(f"rading_mode]Trades: {trades} |}")
        print(f"Trades: {trades} | ✅ {wins}W | ✅ {wins}W | ❌ {losses ❌ {losses}L")
       }L")
        if trades >  if trades > 0:
            print0:
            print(f"Win Rate: {wins(f"Win Rate: {wins/trades*100:.1f}%")
        print/trades*100:.(f"💰 Initial1f}%")
        print(f"💰 Initial: ${initial:.2f} → Final: ${final: ${initial:.2f} →:.2f Final: ${final:.2f}")
        print(f"💚 Profit:}")
        print(f"💚 Profit: +${profit:.2f} | 💔 Loss: +${profit:. -${loss_2f} | 💔 Loss: -${loss_amt:.2f}")
        print(famt:.2f}")
        print(f"📊 Net: ${final_p"📊 Net: ${final_pnl:+.2nl:+.2f}")
        print(f"{'='f}")
        print(f"{'='*60}")
*60}")
...
async def main():
    try:
        await trade main():
    try:
        await trade_and_monitor_and_monitor()
    finally:
        await cleanup()
       ()
    finally:
        await cleanup()
        pending = [t pending = [t for t in asyncio.all_t for t in asasks() if tyncio.all_tasks() if t is not asyn is not asyncio.current_task()]
        for taskcio.current_task()]
        for task in in pending:
            task.cancel()
        pending:
            task.cancel()
        if pending:
            await asyncio.gather(*pending if pending:
            await asyncio.gather(*pending, return_exceptions=True)


if __name__ == "__, return_exceptions=True)


if __name__ == "__main__":
    try:
        asyncio.run(mainmain__":
    try:
        asyncio.run(main())
    except Keyboard())
