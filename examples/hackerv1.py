...
With OTC Only Mode & Timezone Selection (IST / BST)
"""
...
# ================= TIMEZONE SELECTION =================
TIMEZONE = "Asia/Kolkata"  # Default: India (UTC+5:30)
# Options: "Asia/Kolkata" (UTC+5:30) or "Asia/Dhaka" (UTC+6)
# ====================================================
...
# ================= Exclude list for OTC pairs (original) =================
USER_EXCLUDE_LIST = [
...
# ================= Timezone handling =================
def get_tz():
    """Return the pytz timezone object based on TIMEZONE global"""
    return pytz.timezone(TIMEZONE)

def now_tz():
    """Return current datetime in selected timezone"""
    return datetime.now(get_tz())

def ts_to_dt(ts):
    """Convert timestamp to selected timezone datetime"""
        ...
        if os.path.exists(SAVE_FILE):
            with open(SAVE_FILE, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data, list):
                    ...
                    OTC_ONLY_MODE = settings.get('OTC_ONLY_MODE', OTC_ONLY_MODE)
    ...
    try:
        data_to_save = {
            ...
            "settings": {
                ...
                "OTC_ONLY_MODE": OTC_ONLY_MODE,
                "TIMEZONE": TIMEZONE
            },
            "last_updated": now_tz().isoformat(),
            "version": "8.9_Timezone_OTC_Fix"
        }
        ...
        if not result_date_str:
            try:
                if 'timestamp' in result:
                    dt = datetime.fromtimestamp(result['timestamp'], get_tz())
...
{header_border} 【 𝐀𝙸 𝐒𝙸𝙶𝙽𝙰𝐋 𝐐𝚇 】{header_border}"""
        ...
        if first_equal:
            ...
            today_trades = [r for r in confirmed_trades if r.get('date') == now_tz().date().isoformat()]
...
# ================= get_profitable_pairs (with OTC Only fix) =================
async def get_profitable_pairs(client, min_profit=70):
    ...
    if OTC_ONLY_MODE:
        # Use the allowed OTC pairs directly – do NOT rely on API
        print(rainbow_text(f"🟢 OTC Only Mode is ON. Allowed pairs: {', '.join(OTC_ALLOWED_PAIRS)}"))
        # We'll return the allowed list – the analysis will skip any that fail to fetch candles.
        return OTC_ALLOWED_PAIRS

    # Normal mode (original logic)
    if CUSTOM_PAIRS_MODE and CUSTOM_PAIRS_LIST:
        print(rainbow_text(f"✅ Custom Pairs Mode ON. Analysing {len(CUSTOM_PAIRS_LIST)} custom pairs."))
    ...
    if not pairs:
        print(rainbow_text("No pairs found with min_profit, using default pairs."))
...
async def send_signal_fire_and_forget(client):
    ...
    try:
        ...
        print(rainbow_text(f"🔍 Analysing {len(pairs[:40])} pairs one by one..."))
    ...
    for attempt in range(max_retries):
        try:
            ...
            if success:
                ...
                print(rainbow_text(f"✅ Account mode set to {ACCOUNT_MODE}"))
                # Fetch balance
                balance = await client.get_balance()
                CURRENT_BALANCE = balance
                print(rainbow_text(f"💰 Balance: {CURRENT_BALANCE:.2f} USD"))
...
🕐 *Start Time:* {now_tz().strftime('%H:%M:%S')}
    ...
    while bot_running:
        try:
            if not client.check_connect():
                ...
                balance = await client.get_balance()
                CURRENT_BALANCE = balance
                print(rainbow_text(f"💰 Balance updated: {CURRENT_BALANCE:.2f} USD"))
...
📊 *Final Statistics:*
✅ Today's Wins: {sum(1 for r in trade_results if r.get('result') == 'Profit' and r.get('date') == now_tz().date().isoformat())}
❌ Today's Losses: {sum(1 for r in trade_results if r.get('result') == 'Loss' and r.get('date') == now_tz().date().isoformat())}
📈 Total Signals Sent: {len([r for r in trade_results if r.get('date') == now_tz().date().isoformat()])}
...
📱 *Contact:* {SIGNAL_USERNAME}
🕐 *Stop Time:* {now_tz().strftime('%H:%M:%S')}
📅 *Date:* {now_tz().strftime('%d/%m/%Y')}
...
def send_manual_result():
    ...
    if choice in ("1", "2"):
        ...
        today_trades_before = [t for t in temp_trades if t.get('date') == now_tz().date().isoformat()]
        ...
        if choice == "1":
            result = "Profit"
            wins += 1
            result_text = "✅✅✅ SURESHOT ✅✅✅"
        elif choice == "2":
            result = "Loss"
            losses += 1
            result_text = "✖️✖️✖️ LOSS ✖️✖️✖️"
...
╭━━━━━━━【⛨】━━━━━━━╮
🔥 Wins:{wins_formatted} | Losses:{losses_formatted} ◈ ({win_rate_formatted}％)
╰━━━━━━━【⛨】━━━━━━━╯
📊 TODAY: {wins_formatted}W/{losses_formatted}L ({win_rate_formatted}％)
🌐 Contact: {fancy_font(SIGNAL_USERNAME)}
...
def settings_menu():
    ...
    while True:
        ...
        options = [
            ...
            f"𝟻𝟸. Save Settings",
            f"𝟻𝟹. 🤖 Auto-Trade: {'✅ ON' if AUTO_TRADE else '❌ OFF'}",
            f"𝟻𝟺. 💰 Trade Amount: ${TRADE_AMOUNT:.2f}",
            f"𝟻𝟻. 💳 Account Mode: {ACCOUNT_MODE}",
            f"𝟻𝟼. 🟢 OTC Only Mode: {'✅ ON' if OTC_ONLY_MODE else '❌ OFF'}",
            f"𝟻𝟽. 🌐 Timezone: {TIMEZONE}",
            f"𝟻𝟾. Back to Main Menu"
        ]
        ...
        if choice == "1":
            ...
            if confirm == 'yes':
                ...
                today_date_str = now_tz().date().isoformat()
            ...
            print(rainbow_text(f"✅ Auto-Trade is now {status}"))
        elif choice == "54":
            try:
                new_amount = float(input(fancy_font(f"Enter new trade amount (Current: ${TRADE_AMOUNT:.2f}): ")))
                if new_amount > 0:
                    TRADE_AMOUNT = new_amount
                    print(rainbow_text(f"✅ Trade Amount set to ${TRADE_AMOUNT:.2f}"))
                else:
                    print(rainbow_text("❌ Amount must be positive."))
            except:
                print(rainbow_text("❌ Invalid amount."))
        elif choice == "55":
            ACCOUNT_MODE = "REAL" if ACCOUNT_MODE == "PRACTICE" else "PRACTICE"
            print(rainbow_text(f"✅ Account Mode set to {ACCOUNT_MODE}. Please restart the bot for changes to take effect."))
        elif choice == "56":
            OTC_ONLY_MODE = not OTC_ONLY_MODE
            status = 'ON' if OTC_ONLY_MODE else 'OFF'
            print(rainbow_text(f"✅ OTC Only Mode is now {status}. Please restart the bot for changes to take effect."))
        elif choice == "57":
            # Toggle timezone between India and Bangladesh
            if TIMEZONE == "Asia/Kolkata":
                TIMEZONE = "Asia/Dhaka"
            else:
                TIMEZONE = "Asia/Kolkata"
            print(rainbow_text(f"✅ Timezone set to {TIMEZONE}. Please restart the bot for changes to take effect."))
        elif choice == "58":
            clear_screen()
            break
        else:
...
def main_menu():
    clear_screen()
    print_hacker_logo()  # <--- ADDED HACKER LOGO IN GREEN
    print("\n" + "="*60)
    ...
    while True:
        try:
            clear_screen()
            print_hacker_logo()  # <--- HACKER LOGO EVERY TIME MAIN MENU REFRESHES
            print("\n" + "="*60)
            ...
            stats = [
                ...
                f"🟢 OTC Only: {'ON' if OTC_ONLY_MODE else 'OFF'}",
                f"🌐 Timezone: {TIMEZONE}",
                f"💵 Current Balance: ${CURRENT_BALANCE:.2f}" if CURRENT_BALANCE > 0 else "💵 Balance: Not connected"
            ]
    ...
    try:
        if sys.platform == "win32":
            ...
            try:
                ...
                pass
        clear_screen()
        print_hacker_logo()  # <--- LOGO AT THE VERY START
        print(rainbow_text("="*70))
        print(rainbow_text("     🤖 TRADEIQDEV 𝐱 𝐀𝙸 𝐁𝐎𝐓 - Professional Trading System"))
        print(rainbow_text("              Version 8.10 - OTC Only & Timezone Selector"))
        print(rainbow_text("="*70))
