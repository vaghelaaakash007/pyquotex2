...
async def martingale_apply(amount, asset_name, direction, duration, balance, martingale_quantity):
    while martingale_quantity > 0:
        balance -= amount
        print(f"🔄 Martingale: ${amount} {direction} {duration}s")
                ...
                while True:
                    ...
                    if status:
                        ...
                        balance, profit, success = await martingale_apply(
                            amount * 2, asset_name, direction, duration, balance, martingale_quantity
                        )

                        if success:
                            total_wins += 1
                            print(f"✅ Martingale Win | Profit: ${profit:.2f}")
                            
                            # ===== UPDATE AMOUNT & WIN STREAK (Martingale Win) =====
                            amount, win_streak = get_next_amount(amount, base_amount, True, win_streak)
                            print(f"💰 Next Amount: ${amount} | Streak: {win_streak}")
                        else:
                            total_losses += 1
