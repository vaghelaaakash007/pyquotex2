...
async def shuffle_mode(config):
    ...
    # Pre-warm strategy with some data if needed
    # We'll fetch data per trade
    ...
    while True:
        ...
        if result == "Win":
            ...
            total_profit += profit
            print(f"   ✅ WIN! +${profit:.2f} | Bal: ${balance:.2f} | {total_wins}W/{total_losses}L")
            # 🟢 SHUFFLE uses 1.5× multiplier (changed from 2×)
            ...
            total_loss_amount += loss_amount
            print(f"   ❌ LOSS! -${loss_amount:.2f} | Bal: ${balance:.2f} | {total_wins}W/{total_losses}L")
            # Loss resets to base amount (multiplier not used on loss)
            amount, win_streak = get_next_amount(amount, base_amount, False, win_streak)
            print(f"   💥 RESET → ${amount}")
