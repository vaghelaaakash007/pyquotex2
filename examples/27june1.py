...
TIMEZONE: UTC+5:30 (IST) - OTC Only Mode
"""

import os
import sys

# ================= TERMUX / KALI LINUX COMPATIBILITY =================
if os.name == 'posix':
...
UTC_PLUS_5_30 = pytz.timezone('Asia/Kolkata')  # India Standard Time

# ================= NEW AUTO-TRADE & ACCOUNT MODE =================
AUTO_TRADE = False          # Set to True to enable automatic trade execution
TRADE_AMOUNT = 10.0         # Base amount in USD
TRADE_DURATION = 60         # Seconds (matches TIMEFRAME)
ACCOUNT_MODE = "PRACTICE"   # "REAL" or "PRACTICE" (demo)
CURRENT_BALANCE = 0.0       # Will be updated on connection
# ================================================================
...
OTC_ALLOWED_PAIRS = [
    ...
    "USDCHF_otc", "USDCAD_otc", "AUDUSD_otc", "NZDUSD_otc"
]
# ================================================================
...
def ts_to_dt(ts):
    """Convert timestamp to UTC+5:30 datetime"""
    if ts is None: return None
    try:
        ts = int(ts)
        if ts > 1_000_000_000_000: ts = ts / 1000
        dt_utc = datetime.fromtimestamp(ts, tz=timezone.utc)
        return dt_utc.astimezone(UTC_PLUS_5_30)
    except:
        try:
            dt = datetime.fromisoformat(str(ts))
            if dt.tzinfo is None: dt = dt.replace(tzinfo=timezone.utc)
            return dt.astimezone(UTC_PLUS_5_30)
        except: return None
...
# ================= রেইনবো টেক্সট ফাংশন =================
_UNICODE_TO_ASCII_MAP = str.maketrans({
...
def _normalize_unicode(text):
    if not isinstance(text, str):
        text = str(text)
    try:
        return text.translate(_UNICODE_TO_ASCII_MAP)
    except Exception:
...
def load_trade_results():
    ...
    global AUTO_TRADE, TRADE_AMOUNT, ACCOUNT_MODE
    global OTC_ONLY_MODE  # <--- new
        ...
        if os.path.exists(SAVE_FILE):
            with open(SAVE_FILE, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data, list):
                    ...
                    AUTO_TRADE = settings.get('AUTO_TRADE', AUTO_TRADE)
                    TRADE_AMOUNT = settings.get('TRADE_AMOUNT', TRADE_AMOUNT)
                    ACCOUNT_MODE = settings.get('ACCOUNT_MODE', ACCOUNT_MODE)
                    OTC_ONLY_MODE = settings.get('OTC_ONLY_MODE', OTC_ONLY_MODE)  # <--- new
                else:
                    trade_results = []
    ...
    try:
        data_to_save = {
            ...
            "settings": {
                ...
                "OTC_ONLY_MODE": OTC_ONLY_MODE  # <--- new
            },
            "last_updated": datetime.now(UTC_PLUS_5_30).isoformat(),
            "version": "8.7_OTC_Only_Mode"
        }
...
def get_today_results():
    today = datetime.now(UTC_PLUS_5_30).date()
        ...
        if not result_date_str:
            try:
                if 'timestamp' in result:
                    dt = datetime.fromtimestamp(result['timestamp'], UTC_PLUS_5_30)
                    ...
                    dt = dt.astimezone(UTC_PLUS_5_30)
                    result_date_str = dt.date().isoformat()
            except:
                continue
...
def create_chart_image(pair: str, candles: List[Dict], result=None) -> BytesIO:
    ...
    try:
        if CHART_BACKGROUND_IMAGE_PATH and os.path.exists(CHART_BACKGROUND_IMAGE_PATH):
            ...
            draw = ImageDraw.Draw(img)
            print(rainbow_text(f"✅ 𝙱𝚊𝚌𝚔𝚐𝚛𝚐𝚞𝚍 𝚒𝚖𝚊𝚐𝚎 𝚕𝚘𝚊𝚍𝚎𝚍 (𝚃𝚛𝚊𝚗𝚜𝚙𝚊𝚛𝚎𝚗𝚌𝚢: {BACKGROUND_TRANSPARENCY}%)"))
        else:
...
def calculate_gaps(candles):
    ...
    for i in range(1, len(candles)):
        ...
        if curr_open > prev_close * 1.001:
            gaps.append({'type': 'GAP_UP', 'start_price': prev_close, 'end_price': curr_open, 'candle_index': i})
        elif curr_open < prev_close * 0.999:
            gaps.append({'type': 'GAP_DOWN', 'start_price': prev_close, 'end_price': curr_open, 'candle_index': i})
    return gaps
    ...
    for i in range(lookback, len(candles) - lookback):
        ...
        for j in range(1, lookback + 1):
            if candles[i-j]['low'] < current_low or candles[i+j]['low'] < current_low:
                ...
                break
        if is_swing_high:
            reversal_areas.append({'type': 'SWING_HIGH', 'price': current_high, 'candle_index': i, 'timestamp': candles[i]['dt'] if 'dt' in candles[i] else None})
        if is_swing_low:
            reversal_areas.append({'type': 'SWING_LOW', 'price': current_low, 'candle_index': i, 'timestamp': candles[i]['dt'] if 'dt' in candles[i] else None})
...
def detect_price_action_patterns(candles):
    ...
    for i in range(2, len(candles) - 2):
        if (candles[i-1]['close'] < candles[i-1]['open'] and
            ...
            patterns.append({'type': 'BULLISH_ENGULFING', 'candle_index': i, 'strength': (candles[i]['close'] - candles[i-1]['open']) / candles[i-1]['open']})
        elif (candles[i-1]['close'] > candles[i-1]['open'] and
              candles[i]['close'] < candles[i]['open'] and
              candles[i]['open'] > candles[i-1]['close'] and
              candles[i]['close'] < candles[i-1]['open']):
            patterns.append({'type': 'BEARISH_ENGULFING', 'candle_index': i, 'strength': (candles[i-1]['open'] - candles[i]['close']) / candles[i-1]['open']})
            ...
            patterns.append({'type': 'HAMMER', 'candle_index': i, 'strength': (candles[i]['close'] - candles[i]['low']) / (candles[i]['high'] - candles[i]['low'])})
        elif (candles[i]['close'] < candles[i]['open'] and
            ...
            patterns.append({'type': 'SHOOTING_STAR', 'candle_index': i, 'strength': (candles[i]['high'] - candles[i]['open']) / (candles[i]['high'] - candles[i]['low'])})
    return patterns
...
async def safe_fetch(q, pair, period):
    try:
        r = await asyncio.wait_for(q.get_candles(pair, time.time(), MAX_CANDLES, period), timeout=15)
        if r:
            return r
    except:
        pass
    try:
        r = await asyncio.wait_for(q.get_candle_v2(pair, period, MAX_CANDLES), timeout=15)
        if r:
            return r
    except:
        pass
...
# ================= সিগন্যাল প্রক্রিয়াকরণ ফাংশন (with trade retry) =================
async def process_signal_background(client, pair, direction, data):
    try:
        def _run_signal():
            ...
            asyncio.set_event_loop(loop)
            try:
                loop.run_until_complete(process_signal(client, pair, direction, data))
            finally:
                try:
                    loop.close()
                except:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(_run_signal)
...
async def process_signal(client, pair, direction, data):
    ...
    global ADVANCED_ANALYSIS, CUSTOM_LINK, AUTO_TRADE, TRADE_AMOUNT, TRADE_DURATION
    ...
    try:
        ...
        current_dt = datetime.now(UTC_PLUS_5_30)  # Use IST
        ...
        try:
            ...
            elif chart_candles and len(chart_candles) > 0:
        ...
        if ADVANCED_ANALYSIS and data:
            ...
            trend_text = ""
            if trend == "UP_TREND": trend_text = "𝚄𝙿 𝚃𝚁𝙴𝙽𝙳"
            elif trend == "DOWN_TREND": trend_text = "𝙳𝙾𝚆𝙽 𝚃𝚁𝙴𝙽𝙳"
            else: trend_text = "𝚂𝙸𝙳𝙴𝚆𝚈𝚂"
...
╚═━━━━━━━ ◢◤◆◥◣ ━━━━━━━═╝"""
        ...
        link_section = f'🔗 <a href="{CUSTOM_LINK}">𝙲𝚁𝙴𝙰𝚃𝙴 𝙰𝙲𝙲𝙾𝚄𝙽𝚃 𝚀𝚇</a>' if CUSTOM_LINK else "🔗 𝙲𝚁𝙴𝙰𝚃𝙴 𝙰𝙲𝙲𝙾𝚄𝙽𝚃 𝚀𝚇"
...
{header_border} 【 𝐀𝙸 𝐒𝙸𝙶𝙽𝙰𝐋 𝐐𝚇 】{header_border}"""
        ...
        trade_data = {
            ...
            'signal_time': formatted_time, 'signal_timestamp': entry_ts,
            'price': price_val, 'result': None, 'profit': None,
            'martingale_step': 0, 'timestamp': server_now,
            'date': datetime.now(UTC_PLUS_5_30).date().isoformat()
        }
        ...
        # ========== IMPROVED TRADE EXECUTION WITH RETRY & FALLBACK ==========
        buy_info = None
        if AUTO_TRADE:
            # Check if asset is open
            asset_name, asset_data = await client.get_available_asset(pair, force_open=True)
            if asset_data and asset_data[2]:
                print(rainbow_text(f"🚀 Placing {direction} trade on {pair} for {TRADE_AMOUNT} USD"))
                
                # Retry logic (up to 3 attempts)
                for attempt in range(1, 4):
                    try:
                        status, buy_info = await client.buy(TRADE_AMOUNT, pair, direction, TRADE_DURATION)
                        if status:
                            print(rainbow_text(f"✅ Trade placed successfully. ID: {buy_info.get('id')}"))
                            trade_data['buy_id'] = buy_info.get('id')
                            break
                        else:
                            error_msg = buy_info if isinstance(buy_info, str) else str(buy_info)
                            print(rainbow_text(f"❌ Attempt {attempt} failed: {error_msg}"))
                            if "closed" in error_msg.lower() or "not found" in error_msg.lower():
                                print(rainbow_text("⚠️ Asset likely closed or invalid. Skipping trade."))
                                break
                            await asyncio.sleep(2)  # wait before retry
                    except Exception as e:
                        print(rainbow_text(f"⚠️ Exception on attempt {attempt}: {str(e)}"))
                        await asyncio.sleep(2)
                else:
                    # All attempts failed – try fallback method if available
                    print(rainbow_text("🔄 All buy attempts failed. Trying fallback method (buy_digital)..."))
                    try:
                        if hasattr(client, 'buy_digital'):
                            status, buy_info = await client.buy_digital(TRADE_AMOUNT, pair, direction, TRADE_DURATION)
                            if status:
                                print(rainbow_text(f"✅ Digital trade placed successfully. ID: {buy_info.get('id')}"))
                                trade_data['buy_id'] = buy_info.get('id')
                            else:
                                print(rainbow_text(f"❌ Digital trade failed: {buy_info}"))
                        else:
                            print(rainbow_text("❌ buy_digital not available."))
                    except Exception as e:
                        print(rainbow_text(f"❌ Fallback error: {e}"))
            else:
                print(rainbow_text(f"⚠️ Asset {pair} is closed. Skipping trade."))
        # ================================================================
        ...
        if first_equal:
            ...
            today_trades = [r for r in confirmed_trades if r.get('date') == datetime.now(UTC_PLUS_5_30).date().isoformat()]
...
☲☲☲_ 【 𝐀𝙸 𝐒𝙸𝙶𝙽𝙰𝐋 𝐐𝚇 】☲☲☲☲"""
        ...
        if MARTINGALE_STEPS > 0:
            for martingale_step in range(1, MARTINGALE_STEPS + 1):
                ...
                # ========== MARTINGALE TRADE EXECUTION WITH RETRY ==========
                if AUTO_TRADE:
                    martingale_amount = TRADE_AMOUNT * (2 ** martingale_step)
                    print(rainbow_text(f"🔄 Martingale step {martingale_step}: placing {martingale_amount} USD on {pair}"))
                    for attempt in range(1, 4):
                        try:
                            status, buy_info = await client.buy(martingale_amount, pair, direction, TRADE_DURATION)
                            if status:
                                print(rainbow_text(f"✅ Martingale trade placed. ID: {buy_info.get('id')}"))
                                break
                            else:
                                error_msg = buy_info if isinstance(buy_info, str) else str(buy_info)
                                print(rainbow_text(f"❌ Martingale attempt {attempt} failed: {error_msg}"))
                                if "closed" in error_msg.lower() or "not found" in error_msg.lower():
                                    break
                                await asyncio.sleep(2)
                        except Exception as e:
                            print(rainbow_text(f"⚠️ Exception on martingale attempt {attempt}: {str(e)}"))
                            await asyncio.sleep(2)
                    else:
                        print(rainbow_text("⚠️ All martingale attempts failed. Skipping this step."))
                # =============================================================
...
async def try_fetch(client, asset, period):
    try:
        ...
        async def fetch_with_timeout(coro, timeout=15):
            try:
                ...
                return None
        try:
            r = await fetch_with_timeout(client.get_candles(asset, end_time, 60, period), 15)
            if r: return r
        except: pass
        try:
            r = await fetch_with_timeout(client.get_candles(asset, period), 15)
            if r: return r
        except: pass
        try:
            r = await fetch_with_timeout(client.get_candles(asset, end_time, period), 15)
            if r: return r
        except: pass
        try:
            ...
            if func:
                ...
                if asyncio.iscoroutine(r): 
                    r = await fetch_with_timeout(r, 15)
                if r: return r
        except: pass
        try:
        ...
        print(rainbow_text(f"⚠️ Fetch error: {str(e)[:50]}"))
    return None
...
# ================= get_profitable_pairs ফাংশন (MODIFIED for OTC Only Mode) =================
async def get_profitable_pairs(client, min_profit=70):
    global ANALYZE_ALL_PAIRS, CUSTOM_PAIRS_MODE, CUSTOM_PAIRS_LIST, OTC_ONLY_MODE

    if OTC_ONLY_MODE:
        # If OTC Only mode is enabled, we only allow the predefined OTC pairs
        print(rainbow_text(f"🟢 OTC Only Mode is ON. Allowed pairs: {', '.join(OTC_ALLOWED_PAIRS)}"))
        # We still need to fetch assets to check if they are open, but we filter to allowed pairs.
        # So we get assets from API or default, then filter.
        # However, we can skip fetching all assets and just use the allowed list.
        # But we need to ensure they are valid assets. We'll use the API to get the full list and then filter.
        # If API fails, we use the allowed list directly.
        try:
            assets = None
            ...
            print(rainbow_text(f"𝙴𝚛𝚛𝚘𝚛 𝚐𝚎𝚝𝚝𝚒𝚗𝚐 𝚊𝚜𝚎𝚝𝚜: {e}"))
            assets = None

        if assets:
            # Get all pair names from API, then filter to only allowed OTC pairs
            all_pairs = []
            if isinstance(assets, dict):
                ...
                for a in assets:
                    if isinstance(a, str):
                        ...
                        name = a.get('name') or a.get('displayName') or a.get('title') or a.get('asset_name')
                        if name:
                            all_pairs.append(name)
            # Filter to allowed pairs (case-insensitive)
            allowed_lower = [p.lower() for p in OTC_ALLOWED_PAIRS]
            filtered = [p for p in all_pairs if p.lower() in allowed_lower]
            # Also ensure we include any allowed pair that might not be in API (e.g., if API returns different names)
            # We'll add those that are in OTC_ALLOWED_PAIRS but not in filtered
            for allowed in OTC_ALLOWED_PAIRS:
                if allowed.lower() not in [f.lower() for f in filtered]:
                    filtered.append(allowed)
            filtered = list(dict.fromkeys(filtered))  # remove duplicates
            print(rainbow_text(f"✅ 𝙾𝚃𝙲 𝙾𝚗𝚕𝚢: 𝙵𝚘𝚞𝚗𝚍 {len(filtered)} 𝚊𝚕𝚕𝚘𝚠𝚎𝚍 𝙾𝚃𝙲 𝚙𝚊𝚒𝚛𝚜."))
            return filtered
        else:
            # If API fails, return the allowed list directly
            print(rainbow_text(f"⚠️ 𝙰𝙿𝙸 𝚏𝚊𝚒𝚕𝚎𝚍, 𝚞𝚜𝚒𝚗𝚐 𝚍𝚎𝚏𝚊𝚞𝚕𝚝 𝙾𝚃𝙲 𝚕𝚒𝚜𝚝."))
            return OTC_ALLOWED_PAIRS

    # else: normal mode (original logic)
...
def send_partial_results():
    ...
    bd_time = datetime.now(UTC_PLUS_5_30)
...
async def send_signal(client):
    ...
    for pair in pairs[:40]:
        tasks.append(analyze_pair(client, pair))
        pair_map.append(pair)
    ...
    for i, result in enumerate(results):
        ...
        try:
            if isinstance(result, Exception):
                print(rainbow_text(f"Error processing pair {pair}: {result}"))
                continue
            ...
            print(rainbow_text(f"Error processing pair {pair}: {e}"))
            continue
...
async def continuous_signal_loop(client):
    ...
    while bot_running:
        try:
            now = int(time.time())
            if now - last_signal_time >= next_signal_interval:
                print(rainbow_text(f"🔍 Checking for signals... (interval: {next_signal_interval//60} min)"))
                task = asyncio.create_task(send_signal_fire_and_forget(client))
                def on_task_done(t):
                    try:
                        t.result()
                    except Exception as exc:
                        print(rainbow_text(f"⚠️ Signal task failed: {str(exc)[:80]}"))
            ...
            print(rainbow_text(f"⚠️ Error in continuous loop ({consecutive_errors}): {str(e)[:80]}"))
            if consecutive_errors > 10:
                print(rainbow_text("❌ Too many errors, restarting loop..."))
                consecutive_errors = 0
            await asyncio.sleep(10)
...
async def send_signal_fire_and_forget(client):
    global signal_count
    try:
        print(rainbow_text("🔍 Starting signal analysis..."))
        pairs = await get_profitable_pairs(client, min_profit=70)
        if not pairs:
            print(rainbow_text("No profitable pairs found. Skipping this cycle."))
            await asyncio.sleep(10)
        ...
        print(rainbow_text(f"🔍 Analyzing {len(pairs[:40])} pairs one by one..."))
        ...
        for pair in pairs[:40]:
            ...
            try:
                result = await asyncio.wait_for(analyze_pair(client, pair), timeout=20)
                if result and len(result) == 3:
                    direction, score, data = result
                    if direction and score > 0:
                        viable_pairs.append((pair, direction, score, data))
                        print(rainbow_text(f"   ✅ {pair}: {direction.upper()} (score: {score})"))
                        if score > best_score:
                            best_pair, best_dir, best_score, best_data = pair, direction, score, data
            except asyncio.TimeoutError:
                print(rainbow_text(f"   ⏱️ {pair}: timeout (skipping)"))
                continue
            except Exception as e:
                print(rainbow_text(f"   ⚠️ {pair}: error - {str(e)[:40]}"))
                continue

        print(rainbow_text(f"🔍 Analysis complete. {len(viable_pairs)} viable signals found."))
...
# ================= ট্রেডিং বট ফাংশন (INTERACTIVE LOGIN) =================
async def trading_bot(email, password):
    global last_signal_time, start_time, next_signal_interval, BOT_ALERT_ON, ACCOUNT_MODE, CURRENT_BALANCE

    start_time = int(time.time())
    next_signal_interval = get_next_signal_interval()

    client = Quotex(email=email, password=password, lang="en")
    client.set_account_mode(ACCOUNT_MODE)
    ...
    for attempt in range(max_retries):
        try:
            ...
            if success:
                ...
                print(rainbow_text(f"✅ 𝙰𝚌𝚌𝚘𝚞𝚗𝚝 𝚖𝚘𝚍𝚎 𝚜𝚎𝚝 𝚝𝚘 {ACCOUNT_MODE}"))
                # Fetch balance
                balance = await client.get_balance()
                CURRENT_BALANCE = balance
                print(rainbow_text(f"💰 𝙱𝚊𝚕𝚊𝚗𝚌𝚎: {CURRENT_BALANCE:.2f} USD"))
                # Send Telegram alert with balance
                if BOT_ALERT_ON:
                    balance_msg = f"""✅ *Bot Connected Successfully*

💳 *Account Mode:* {ACCOUNT_MODE}
💰 *Balance:* ${CURRENT_BALANCE:.2f}
👤 *Account:* {email}
🕐 *Time:* {datetime.now(UTC_PLUS_5_30).strftime('%H:%M:%S')}

🚀 *TRADEIQDEV 𝐱 𝐀𝙸 𝐁𝐎𝐓 - Ready to Trade*"""
...
🕐 *Time:* {datetime.now(UTC_PLUS_5_30).strftime('%H:%M:%S')}"""
    ...
    # Continue with the rest of the bot (signal loop) as before...
    if BOT_ALERT_ON:
...
💳 *Account Mode:* {ACCOUNT_MODE}
💰 *Balance:* ${CURRENT_BALANCE:.2f}
🟢 *OTC Only Mode:* {'ON' if OTC_ONLY_MODE else 'OFF'}

📱 *Contact:* {SIGNAL_USERNAME}
🕐 *Start Time:* {datetime.now(UTC_PLUS_5_30).strftime('%H:%M:%S')}
📅 *Date:* {datetime.now(UTC_PLUS_5_30).strftime('%d/%m/%Y')}
    ...
    while bot_running:
        try:
            if not client.check_connect():
                ...
                balance = await client.get_balance()
                CURRENT_BALANCE = balance
                print(rainbow_text(f"💰 𝙱𝚊𝚕𝚊𝚗𝚌𝚎 𝚞𝚙𝚍𝚊𝚝𝚎𝚍: {CURRENT_BALANCE:.2f} USD"))
...
def _thread_target(email, password):
    import asyncio
    loop = None
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(trading_bot(email, password))
    except Exception as e:
...
def start_bot(email, password):
    ...
    t = threading.Thread(target=_thread_target, args=(email, password), daemon=True)
    t.start()
    ...
    print(rainbow_text("   ⚡ Running in BACKGROUND - you can use the menu while bot trades!"))
    print(rainbow_text("   ⏰ Signals will be sent every 3-5 minutes automatically."))
...
📊 *Final Statistics:*
✅ Today's Wins: {sum(1 for r in trade_results if r.get('result') == 'Profit' and r.get('date') == datetime.now(UTC_PLUS_5_30).date().isoformat())}
❌ Today's Losses: {sum(1 for r in trade_results if r.get('result') == 'Loss' and r.get('date') == datetime.now(UTC_PLUS_5_30).date().isoformat())}
📈 Total Signals Sent: {len([r for r in trade_results if r.get('date') == datetime.now(UTC_PLUS_5_30).date().isoformat()])}
...
📱 *Contact:* {SIGNAL_USERNAME}
🕐 *Stop Time:* {datetime.now(UTC_PLUS_5_30).strftime('%H:%M:%S')}
📅 *Date:* {datetime.now(UTC_PLUS_5_30).strftime('%d/%m/%Y')}
...
def send_manual_result():
    ...
    if choice in ("1", "2"):
        ...
        today_trades_before = [t for t in temp_trades if t.get('date') == datetime.now(UTC_PLUS_5_30).date().isoformat()]
...
☲☲☲_ 【 𝐀𝙸 𝐒𝙸𝙶𝙽𝙰𝐋 𝐐𝚇 】☲☲☲☲"""
        ...
        safe_async_run(send_telegram_photo_or_chart(result_msg, last_trade['pair'], None, result=result))
...
def safe_async_run(coro):
    """Safely run coroutine from sync code - works in Termux/Kali"""
    try:
...
def settings_menu():
    ...
    while True:
        ...
        options = [
            ...
            f"𝟻𝟸. 𝚂𝚊𝚟𝚎 𝚂𝚎𝚝𝚝𝚒𝚗𝚐𝚜",
            f"𝟻𝟹. 🤖 𝙰𝚞𝚝𝚘-𝚃𝚛𝚊𝚍𝚎: {'✅ 𝙾𝙽' if AUTO_TRADE else '❌ 𝙾𝙵𝙵'}",
            f"𝟻𝟺. 💰 𝚃𝚛𝚊𝚍𝚎 𝙰𝚖𝚘𝚞𝚗𝚝: ${TRADE_AMOUNT:.2f}",
            f"𝟻𝟻. 💳 𝙰𝚌𝚌𝚘𝚞𝚗𝚝 𝙼𝚘𝚍𝚎: {ACCOUNT_MODE}",
            f"𝟻𝟼. 🟢 𝙾𝚃𝙲 𝙾𝚗𝚕𝚢 𝙼𝚘𝚍𝚎: {'✅ 𝙾𝙽' if OTC_ONLY_MODE else '❌ 𝙾𝙵𝙵'}",
            f"𝟻𝟽. 𝙱𝚊𝚌𝚔 𝚝𝚘 𝙼𝚊𝚒𝚗 𝙼𝚎𝚗𝚞"
        ]
        ...
        if choice == "1":
            ...
            if confirm == 'yes':
                ...
                today_date_str = datetime.now(UTC_PLUS_5_30).date().isoformat()
            ...
            status = '𝙾𝙽' if AUTO_TRADE else '𝙾𝙵𝙵'
            print(rainbow_text(f"✅ 𝙰𝚞𝚝𝚘-𝚃𝚛𝚊𝚍𝚎 𝚒𝚜 𝚗𝚘𝚠 {status}"))
        elif choice == "54":
            try:
                new_amount = float(input(fancy_font(f"𝙴𝚗𝚝𝚎𝚛 𝚗𝚎𝚠 𝚝𝚛𝚊𝚍𝚎 𝚊𝚖𝚘𝚞𝚗𝚝 (𝙲𝚞𝚛𝚛𝚎𝚗𝚝: ${TRADE_AMOUNT:.2f}): ")))
                if new_amount > 0:
                    TRADE_AMOUNT = new_amount
                    print(rainbow_text(f"✅ 𝚃𝚛𝚊𝚍𝚎 𝙰𝚖𝚘𝚞𝚗𝚝 𝚜𝚎𝚝 𝚝𝚘 ${TRADE_AMOUNT:.2f}"))
                else:
                    print(rainbow_text("❌ 𝙰𝚖𝚘𝚞𝚗𝚝 𝚖𝚞𝚜𝚝 𝚋𝚎 𝚙𝚘𝚜𝚒𝚝𝚒𝚟𝚎."))
            except:
                print(rainbow_text("❌ 𝙸𝚗𝚟𝚊𝚕𝚒𝚍 𝚊𝚖𝚘𝚞𝚗𝚝."))
        elif choice == "55":
            ACCOUNT_MODE = "REAL" if ACCOUNT_MODE == "PRACTICE" else "PRACTICE"
            print(rainbow_text(f"✅ 𝙰𝚌𝚌𝚘𝚞𝚗𝚝 𝙼𝚘𝚍𝚎 𝚜𝚎𝚝 𝚝𝚘 {ACCOUNT_MODE}. Please restart the bot for changes to take effect."))
        elif choice == "56":
            OTC_ONLY_MODE = not OTC_ONLY_MODE
            status = '𝙾𝙽' if OTC_ONLY_MODE else '𝙾𝙵𝙵'
            print(rainbow_text(f"✅ 𝙾𝚃𝙲 𝙾𝚗𝚕𝚢 𝙼𝚘𝚍𝚎 𝚒𝚜 𝚗𝚘𝚠 {status}. Please restart the bot for changes to take effect."))
        elif choice == "57":
            clear_screen()
            break
        else:
    ...
    while True:
        try:
            ...
            stats = [
                ...
                f"🤖 𝙰𝚞𝚝𝚘-𝚃𝚛𝚊𝚍𝚎: {'𝙾𝙽' if AUTO_TRADE else '𝙾𝙵𝙵'}",
                f"💰 𝚃𝚛𝚊𝚍𝚎 𝙰𝚖𝚘𝚞𝚗𝚝: ${TRADE_AMOUNT:.2f}",
                f"💳 𝙰𝚌𝚌𝚘𝚞𝚗𝚝 𝙼𝚘𝚍𝚎: {ACCOUNT_MODE}",
                f"🟢 𝙾𝚃𝙲 𝙾𝚗𝚕𝚢: {'𝙾𝙽' if OTC_ONLY_MODE else '𝙾𝙵𝙵'}",
                f"💵 𝙲𝚞𝚛𝚛𝚎𝚗𝚝 𝙱𝚊𝚕𝚊𝚗𝚌𝚎: ${CURRENT_BALANCE:.2f}" if CURRENT_BALANCE > 0 else "💵 𝙱𝚊𝚕𝚊𝚗𝚌𝚎: 𝙽𝚘𝚝 𝚌𝚘𝚗𝚗𝚎𝚌𝚝𝚎𝚍"
            ]
            ...
            if choice == "1" and not bot_running:
                ...
                print(rainbow_text("\n🔑 Enter your Quotex login credentials:"))
                email = input(rainbow_text("Email: ")).strip()
                password = input(rainbow_text("Password: ")).strip()
                if not email or not password:
                    print(rainbow_text("❌ Email and password are required."))
                    input(rainbow_text("\n𝙿𝚛𝚎𝚜𝚜 𝙴𝚗𝚝𝚎𝚛 𝚝𝚘 𝚌𝚘𝚗𝚝𝚒𝚗𝚞𝚎..."))
                    continue
...
if __name__ == "__main__":
    try:
        ...
        print(rainbow_text("     🤖 TRADEIQDEV 𝐱 𝐀𝙸 𝐁𝐎𝐓 - 𝙿𝚛𝚘𝚏𝚎𝚜𝚜𝚒𝚘𝚗𝚊𝚕 𝚃𝚛𝚊𝚍𝚒𝚗𝚐 𝚂𝚢𝚜𝚝𝚎𝚖"))
        print(rainbow_text("              𝚅𝚎𝚛𝚜𝚒𝚘𝚗 𝟾.𝟾 - 𝙾𝚃𝙲 𝙾𝚗𝚕𝚢 𝙼𝚘𝚍𝚎"))
        print(rainbow_text("="*70))
