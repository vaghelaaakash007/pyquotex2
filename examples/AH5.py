...
class Stochastic_Scalping(ScalpingStrategyBase):
    ...
    def get_signal(self):
        ...
        prices = list(self.price_history)
        ema_21 = self.calculate_ema(prices, 21)
        k, _ = self.calculate_stochastic(prices)
        current_price = prices[-1]
...
async def select_assets(client):
    """Show asset menu with payouts and return the selected asset list."""
    # Define the four main assets
    main_assets = ["AUDCAD_otc", "USDJPY_otc", "USDCAD_otc", "USDBRL_otc"]
    
    print("\n" + "=" * 60)
    print("💱 ASSET SELECTION")
    print("=" * 60)
    print("Choose the assets to trade (enter numbers separated by commas):")
    print("  (Payout percentages are shown for each asset)")
    print("-" * 60)

    # Fetch payout for each asset
    payouts = {}
    for asset in main_assets:
        try:
            payout = client.get_payout_by_asset(asset)
            payouts[asset] = payout if payout else 0.0
        except Exception:
            payouts[asset] = 0.0

    # Display menu
    for idx, asset in enumerate(main_assets, start=1):
        payout_str = f"{payouts[asset]:.1f}%" if payouts[asset] > 0 else "N/A"
        print(f"  {idx}. {asset:<12} - Payout: {payout_str}")

    print(f"  5. ALL (all available assets)")
    print("=" * 60)

    while True:
        choice = input("\n👉 Enter your choice(s) (e.g., 1,2,4 or 5): ").strip()
        if choice.upper() == "5" or choice == "ALL":
            # Fetch all assets
            try:
                ...
                print(f"✅ Selected ALL – {len(asset_list)} assets found.")
                # Remove duplicates (just in case)
                seen = set()
                asset_list = [x for x in asset_list if not (x in seen or seen.add(x))]
                break
            except Exception as e:
                print(f"⚠️ Could not fetch all assets: {e}. Please try again.")
                continue

        else:
            # Parse comma-separated numbers
            parts = [p.strip() for p in choice.split(",") if p.strip().isdigit()]
            if not parts:
            ...
            for p in parts:
                num = int(p)
                if 1 <= num <= 4:
                    selected.append(main_assets[num - 1])
                else:
                    print(f"❌ Invalid number: {num}. Allowed: 1-4 or 5.")
                    invalid = True
                    break
            ...
            seen = set()
            asset_list = [x for x in selected if not (x in seen or seen.add(x))]
            print(f"✅ Selected: {', '.join(asset_list)}")
            break

    print(f"📌 Final asset list: {', '.join(asset_list)}")
    return asset_list
...
async def check_result(buy_data, direction, asset_name=None):
    ...
    if open_price is None:
        try:
            win_status, _ = await client.check_win(buy_data["id"])
