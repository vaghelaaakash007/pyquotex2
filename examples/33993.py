...
def get_next_amount(base_amount, is_win, loss_streak, prev_amount):
    ...
    - Win: reset to base, loss_streak = 0.
    - Loss:
        * 1st loss: base * 2.0
        * 2nd loss: prev_amount * 2.4
        * 3rd loss: prev_amount * 2.5
        * 4th loss: prev_amount * 2.6
        * 5th loss: reset to base (loss_streak = 0)
    """
    if is_win:
        # Reset to base on every win
        ...
        if loss_streak >= 5:
            ...
            # Losses 2, 3, 4: multiply previous amount by specific factor
            if loss_streak == 2:
                factor = 2.4
            elif loss_streak == 3:
                factor = 2.5
...
async def single_asset_mode(config):
    """Single asset trading with updated money management"""
    ...
    total_loss_amount = 0
    loss_streak = 0
    last_trade_amount = amount  # used for multipliers on losses
    ...
    while True:
        ...
        if result == "Win":
            ...
            # On win: reset to base and reset loss streak
        ...
        # Save current amount as previous for next calculation
        prev = amount
...
async def shuffle_mode(config):
    """Shuffle currencies mode with updated money management"""
    max_trades, base_amount, stop_loss, stop_profit, strategy_manager, _ = config
...
async def multi_market_mode(config):
    """Multi-market scanning & trading with updated money management"""
    ...
    # State for money management across all trades
    loss_streak = 0
    last_trade_amount = base_amount
    current_amount = base_amount  # will be updated after each trade outcome
        ...
        while True:
            ...
            if signals:
                ...
                for signal in signals[:3]:
                    ...
                    # Create trade with current_amount
            ...
            # Check completed trades and update money management
                ...
                try:
                    ...
                    if result:
                        if result["win"]:
                            ...
                            # Loss: update using previous amount (current_amount before trade)
                            prev = current_amount  # the amount that was used
...
async def execute_multi_trade(signal, amount, scanner):
    """Execute one multi-market trade with given amount"""
    asset = signal["asset"]
